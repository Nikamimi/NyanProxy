...
class ThreadManager:
    ...
    def create_thread_pool(self, pool_name: str, max_workers: int = 10):
        ...
        with self.lock:
            ...
            print(f"[THREAD] Created thread pool '{pool_name}' with {max_workers} workers")
            return pool
    ...
    def create_daemon_thread(self, target, name: str = None, args: tuple = ()):
        ...
        def wrapped_target(*args):
            try:
                ...
                print(f"[WARN] Thread {thread_id} error: {e}")
            finally:
                with self.lock:
                    if thread_id in self.active_threads:
                        del self.active_threads[thread_id]
                        print(f"[CLEAN] Cleaned up thread {thread_id}")
        ...
        thread.start()
        print(f"[ROCKET] Started managed thread: {thread_id}")
        return thread_id
    ...
    def cleanup_dead_threads(self):
        ...
        with self.lock:
            ...
            if dead_threads:
                print(f"[CLEAN] Cleaned up {len(dead_threads)} dead threads")
    ...
    def shutdown_all(self):
        ...
        print("[STOP] Shutting down thread manager...")
        self.shutdown_event.set()
        
        # Shutdown thread pools
        for pool_name, pool in self.executor_pools.items():
            print(f"[STOP] Shutting down pool: {pool_name}")
            pool.shutdown(wait=True, cancel_futures=True)
        
        print("[OK] Thread manager shutdown complete")
    ...
    def get_session(self, service: str):
        ...
        with self.lock:
            if service not in self.sessions:
                ...
                print(f"[LINK] Created connection pool for {service}")
    ...
    def close_all_sessions(self):
        ...
        with self.lock:
            for service, session in self.sessions.items():
                try:
                    session.close()
                    print(f"[LINK] Closed connection pool for {service}")
                except Exception as e:
                    print(f"[WARN] Error closing session for {service}: {e}")
            self.sessions.clear()
...
class APIKeyManager:
    ...
    def get_api_key(self, service: str, exclude_failed: bool = True, exclude_key: str = None) -> str:
        ...
        if service not in self.api_keys or not self.api_keys[service]:
            print(f"[WARN] No API keys available for {service}")
            return None
        ...
        if not available_keys:
            ...
            if exclude_failed:
                print(f"[RETRY] All {service} keys failed, trying with failed keys...")
                return self.get_api_key(service, exclude_failed=False, exclude_key=exclude_key)
            print(f"[FAIL] No available keys for {service}")
            return None
        ...
        print(f"[KEY] Selected key {selected_key[:8]}...{selected_key[-4:]} for {service} ({key_index + 1}/{len(available_keys)} available)")
        return selected_key
    ...
    def permanently_remove_key(self, service: str, key: str, reason: str):
        ...
        with self.lock:
            if service in self.api_keys and key in self.api_keys[service]:
                ...
                print(f"[DELETE] PERMANENTLY REMOVED key {key[:8]}...{key[-4:]} from {service} pool (reason: {reason})")
                print(f"[KEY] {service} pool now has {len(self.api_keys[service])} keys remaining")
    ...
    def handle_api_error(self, service: str, key: str, error_message: str, status_code: int = None) -> bool:
        ...
        if status_code and error_message:
            ...
            print(f"[SEARCH] Error classified as: {error_type}, retryable: {is_retryable}")
            ...
            # Handle temporary failures - add to failed keys set
                ...
                print(f"[YELLOW] Temporarily marking {key[:8]}...{key[-4:]} as failed for {service}")
                self.failed_keys.add(key)
                return True
...
def openai_chat_completions():
    ...
    if model_info and (model_info.max_input_tokens or model_info.max_output_tokens):
        print(f"[SEARCH] Token validation for model {model}: max_input={model_info.max_input_tokens}, max_output={model_info.max_output_tokens}")
        ...
        try:
            ...
            print(f"[SEARCH] Raw unified tokenizer result: {token_result}")
            # Check for both possible key names
            input_tokens = token_result.get('input_tokens', 0) or token_result.get('prompt_tokens', 0)
            print(f"[CHART] Unified tokenizer result: {input_tokens} tokens")
        except Exception as token_error:
            print(f"[WARN] Unified tokenizer failed: {token_error}")
            import traceback
            traceback.print_exc()
        
        # If unified tokenizer returned 0, use basic fallback
        if input_tokens == 0:
            print(f"[WARN] Unified tokenizer returned 0, using character estimation...")
            messages = request_json.get('messages', [])
            total_chars = sum(len(str(msg.get('content', ''))) for msg in messages)
            input_tokens = max(1, total_chars // 4)  # Conservative 4 chars per token
            print(f"[CHART] Character fallback: {input_tokens} tokens (from {total_chars} chars)")
        
        print(f"[TARGET] Final input token count: {input_tokens}")
        ...
        print(f"[OK] Validation result: {is_valid}, message: {error_message}")
        
        if not is_valid:
            print(f"[FAIL] Token limit exceeded, returning error")
    ...
    for attempt in range(max_retries):
        ...
        if not api_key:
            print(f"[FAIL] No API keys available for OpenAI on attempt {attempt + 1}")
            metrics.track_request('chat_completions', time.time() - start_time, error=True)
            return jsonify({"error": "No OpenAI API key configured"}), 500
        
        used_keys.append(api_key)
        print(f"[RETRY] Attempt {attempt + 1}/{max_retries} using key {api_key[:8]}...{api_key[-4:]}")
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                print(f"[OK] OpenAI API call successful on attempt {attempt + 1}")
                print(f"[KEY] Keys tried: {[k[:8] + '...' + k[-4:] for k in used_keys]}")
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if 'usage' in response_data:
                        ...
                        print(f"[CAT] Token tracking for {model}: input={tokens.get('prompt_tokens', 0)}, output={tokens.get('completion_tokens', 0)} (API provided)")
                ...
                if is_streaming and tokens is None:
                    try:
                            ...
                            try:
                                ...
                                if complete_response.strip():
                                    ...
                                    try:
                                        ...
                                        print(f"[CAT] Token tracking for {model}: input={token_result['prompt_tokens']}, output={completion_tokens} (tiktoken on complete response)")
                                    except Exception:
                                        ...
                                        completion_tokens = completion_token_result.get('completion_tokens', 0)
                                        print(f"[CAT] Token tracking for {model}: input={token_result['prompt_tokens']}, output={completion_tokens} (unified tokenizer fallback)")
                        ...
                        print(f"[CAT] DEBUG: Streaming request for {model} - estimated {completion_tokens} completion tokens")
                        
                    except Exception as e:
                        print(f"[ERROR] DEBUG: Error counting tokens for streaming {model}: {e}")
                        pass
                elif tokens is None:
                    ...
                    try:
                        ...
                        print(f"[CAT] Token tracking for {model}: input={tokens['prompt_tokens']}, output={tokens['completion_tokens']} (unified tokenizer)")
                ...
                try:
                    if conversation_logger.enabled and response.status_code == 200:
                        ...
                        try:
                            ...
                            print(f"[ERROR] DEBUG: Could not extract response content for logging: {e}")
...
def test_google_sheets_connection():
    ...
    try:
        ...
        try:
            import gspread
            from google.oauth2.service_account import Credentials
            print("[CHART] Google Sheets modules imported successfully")
        except ImportError as e:
        ...
        try:
            ...
            print("[CHART] Service account JSON parsed successfully")
            ...
            print("[CHART] Credentials created successfully")
            
            # Initialize client
            client = gspread.authorize(credentials)
            print("[CHART] Client authorized successfully")
            
            # Try to access the spreadsheet
            spreadsheet = client.open_by_key(spreadsheet_id)
            print(f"[CHART] Successfully accessed spreadsheet: '{spreadsheet.title}'")
            
            # Try to list worksheets
            worksheets = spreadsheet.worksheets()
            worksheet_names = [ws.title for ws in worksheets]
            print(f"[CHART] Found {len(worksheets)} worksheets: {worksheet_names}")
...
def dashboard():
    ...
    for service, keys in key_manager.api_keys.items():
        ...
        # [ROCKET] NEW: Use enhanced key health stats method
    ...
    dashboard_config = {
        'title': f"{os.getenv('BRAND_EMOJI', '[CAT]')} {os.getenv('DASHBOARD_TITLE', 'NyanProxy Dashboard')}",
...
def anthropic_messages():
    ...
    if model_info and (model_info.max_input_tokens or model_info.max_output_tokens):
        ...
        try:
            ...
            print(f"[SEARCH] Raw unified tokenizer result (Anthropic): {token_result}")
            # Check for both possible key names
            input_tokens = token_result.get('input_tokens', 0) or token_result.get('prompt_tokens', 0)
            print(f"[CHART] Unified tokenizer result: {input_tokens} tokens")
        except Exception as token_error:
            ...
            print(f"[CHART] Character fallback for Anthropic: {input_tokens} tokens")
    ...
    for attempt in range(max_retries):
        ...
        if not api_key:
            print(f"[FAIL] No API keys available for Anthropic on attempt {attempt + 1}")
            metrics.track_request('chat_completions', time.time() - start_time, error=True)
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                # Success - update key health
                print(f"[OK] Anthropic API call successful on attempt {attempt + 1}")
                print(f"[KEY] Keys tried: {[k[:8] + '...' + k[-4:] for k in used_keys]}")
                key_manager.update_key_health(api_key, True)
...
def google_gemini_proxy(model_path):
    ...
    try:
                ...
                if is_streaming:
                    ...
                    def generate():
                        try:
                            ...
                            print(f"[FAIL] Streaming error: {str(e)}")
                            import traceback
                            traceback.print_exc()
        ...
        print(f"[FAIL] Google Gemini proxy error: {str(e)}")
...
def google_models():
    ...
    try:
        ...
        print(f"[FAIL] Google models error: {str(e)}")
        return jsonify({"error": {"message": "Internal server error", "type": "internal_error"}}), 500
